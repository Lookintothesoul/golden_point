...
import {getExchangeRatesApi, getExchangeRatesOldApi} from "./request/getExchangeRatesApi";
...
export const ExchangeRates = () => {
    ...
    {
        if (current > previous)
            return <span className='up'>{((((current - previous)/previous)*100).toFixed(4) + '% ▲')}</span>
        if (current < previous)
            return <span className='low'>{((((current - previous)/previous)*100).toFixed(4) + '% ▼')}</span>
        return ' ';
    }
    ...
    async function getHistoryRates (charCode) {
        ...
        for (let i = 0; i < 10; i++) {
            await getExchangeRatesOldApi(previousUrl).then(({data}) => {
                previousUrl = data.PreviousURL
                historyRates.list.push({date: data.Date, value: data.Valute[charCode].Value})
    ...
    return (<div>
        <p className="curDate">Курсы валют на {exchangeRates.Date.slice(0,10)}</p>

        <div>
            {Object.values(exchangeRates.Valute).map((valute) =>
                <div key={valute.ID} className="currentValute">
                    <ul className="tableRate" type="none">
                        <li className="Rate" onClick={() => getHistoryRates(valute.CharCode)}>
                            ...
                            <div>
                                {historyRates.list.map(rate =>
                                    <div key={rate.date} className="prevRates">
                                        ...
                                        <span>{rate.value.toFixed(4).replace('.', ',')} руб.</span>
                                    </div>
                                )}
                            </div>)
                        }
